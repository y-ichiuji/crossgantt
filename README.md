...
- **担当者別 / プロジェクト別 / マイルストーン別 / カテゴリ別**のグルーピング。
  大項目・中項目・小項目の 3 段まで重ねられるので、「プロジェクトごとに並べたうえで
  カテゴリに分ける」といった見方ができます
- 日 / 週 / 月のズーム、今日の縦線、土日の背景、日付ごとの罫線
- Backlog のガントチャートと同じく、**バーの色はステータスの色**
- 課題の件名はバーの幅で省略せず、**バーから右へ突き抜けて全文を表示**
- 各行に**担当者のアイコン**を表示し、課題名の末尾に**ステータスのバッジ**（バーと同じ色）を表示
...
デプロイのジョブは GitHub の `production` Environment に属していて、**デプロイできるブランチを
`main` だけに限定**しています。`skip_checks` 付きの `workflow_dispatch` で確認を省略しても、
`main` 以外のコミットが本番へ出ることはありません。

デプロイそのものに承認は挟んでいません。代わりに**依存更新も含めて自動マージを行わない**ことで、
`main` に入る変更を必ず人が見た状態に保っています（デプロイのたびに承認するより、PR を
マージする時点で中身を見るほうが実態に合うためです）。
...
- `main` にマージされ、CI と CodeQL が揃って成功するとデプロイされる
...
- 公開から 7 日を過ぎた版だけを対象にする（脆弱性の修正は例外）
- **自動マージはしない。** `main` に入った変更はそのまま本番のデプロイ対象になるため、
  依存更新も含めて必ず人が中身を見てからマージする
